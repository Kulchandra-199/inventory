...
const registerController = async (req,res) =>{
    ...
    if(password.length < 8){
        return  res.status(400).json({success:false, msg: "password length should be greater than 8!"});
    }

    // check if user is already 
    const oldUser = await User.findOne({email});
    if(oldUser){
        return res.status(403).json({success: false, msg: "This email is already taken!"});
    }
    ...
    bcrypt.genSalt(12, (err, salt)=> {
        bcrypt.hash(password, salt,  async(err,hash) => {
            ...
            if(sendMail){
                res.status(201)
                .json({
                    success: true, msg: "Registeerd successfully! Error in sending verification email please try again later "
                });  
            }else{
                res.status(201)
                .json({
                    success: true, msg: "Registeerd successfully "
                });
            }
 ...
 //send forgot pass email
    ...
    const link = "http://"+req.hostname+ ":5000/api/auth/verifyToken?token"+ token;
    ...
    if(sendMail){
        res.status(201)
        .json({
            success: true, msg: "error in sending email!",
        });  
    }else{
        res.status(201)
        .json({
            success: true, msg: "Rest password email sent!"
        });
    }
   ...
   const resetPasswordController =  async(req,res)=>{
    ...
    bcrypt.genSalt(12, (err, salt) => {
        bcrypt.hash(newPassword, salt, async(err, hash) =>{
           ...
           const updateData = await User.FindOneAndUpdate({email},
            {
                $set:{
                    password: hashedPassword
                }
